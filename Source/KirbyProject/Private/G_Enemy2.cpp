...
void AG_Enemy2::SetState(EEnemyState NewState)
{
	if (CurrentState != NewState)
	{
		...
		case EEnemyState::Dead:
			...
			KnockBack(); // 넉백
			GetWorldTimerManager().SetTimer(TimerHandle, this, &AG_Enemy2::Die, DeadDelay, false); // 애니메이션 재생하고 2초 뒤 죽음
			break;
		default:
			break;
		}
		...
		if (!dir.IsNearlyZero())
		{
			...
			if (AIController)
			{
				if (bCanSpawnNiagara)
				{
					...
					GetWorldTimerManager().SetTimer(TimerHandle, this, &AG_Enemy2::EnableNiagara, NiagaraInterval, false); // 몇 초 간격으로 생성
				}
				// 이동 속도 조절
